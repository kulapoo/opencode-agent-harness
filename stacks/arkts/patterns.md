...
ArkTS uses `interface` for contracts and `class` for DTOs (no `any`, no untyped
object literals). Prefer `Promise`-returning async methods:

```typescript
interface TaskAPI {
  createTask(input: CreateTaskInput): Promise<Task>;
  listTasks(params: ListTasksParams): Promise<Paged<Task>>;
...
Use a typed error class hierarchy; map to the universal REST envelope
(`{ code, message, details? }`) at the network boundary:
...
Validate at the network/data boundary (after parsing JSON into a class);
internal code trusts the typed object. ArkTS forbids `any`, so type every
parsed value explicitly:

```typescript
function parseCreateTask(raw: string): CreateTaskInput {
  const obj: Record<string, Object> = JSON.parse(raw) as Record<string, Object>;
  const title: string = obj['title'] as string;
  if (title.length === 0 || title.length > 200) {
    throw new ValidationError({ field: 'title', reason: '1..200 chars' });
  }
  const description: string | undefined =
    obj['description'] !== undefined ? obj['description'] as string : undefined;
...
- Add new fields as optional (`description?: string`) so existing callers
  compile and old JSON parses.
- Adding a required field is breaking.
- Avoid `any`; prefer explicit `T | undefined` / `T | null`.
- Don't change a field's type or remove a field others read without a
  deprecation window.

## Variant Types

ArkTS supports discriminated unions like TS. Use tagged interfaces for variants
with payloads:

```typescript
interface TaskStatusPending   { type: 'pending' }
interface TaskStatusProgress  { type: 'in_progress'; assignee: string; startedAt: Date }
interface TaskStatusCompleted { type: 'completed'; completedAt: Date; completedBy: string }
interface TaskStatusCancelled { type: 'cancelled'; reason: string; cancelledAt: Date }
...
function label(s: TaskStatus): string {
  switch (s.type) {
    ...
    case 'in_progress': return `In progress (${s.assignee})`;
    case 'completed':   return `Done on ${s.completedAt}`;
...
Separate request classes (caller-provided) from response classes
(server-generated fields included). See `CreateTaskInput` vs `Task` in Contract
Definition above.

## Opaque IDs

Use branded types to keep distinct IDs from being interchangeable:
