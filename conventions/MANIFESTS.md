# Non-JS/TS Manifest → Conventions Dir

Loaded on demand by `init-conventions-declaration` when a non-JS/TS manifest
is detected. JS/TS resolution lives inline in the skill (it has
...
signals a conventions dir that does not exist, fall back to `common/` only and
write in the target language's idioms.
