...
1. **Install the harness into your project.** From inside the target project:
   ...
   ```bash
   curl -fsSL https://raw.githubusercontent.com/kulapoo/opencode-agent-harness/v0.4.1/install.py \
     | python3 - install
   ```

   This fetches the installer pinned to tag `v0.4.1` and runs it once. It
   ...
   **Prefer to inspect first?** Cautious environments (corporate networks, audit
   ...
   ```bash
   curl -fsSL https://raw.githubusercontent.com/kulapoo/opencode-agent-harness/v0.4.1/install.py -o install.py
   less install.py
