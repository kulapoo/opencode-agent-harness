...
Used ONLY by the `init-conventions-declaration` skill to generate
`rules/tech-conventions.md`. Do NOT run this at session start — read
`rules/tech-conventions.md` from the system prompt instead.
...
`Gemfile`→`ruby`, etc.). It's a flat 1:1 language mapping — load it when a
