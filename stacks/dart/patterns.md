...
Use `abstract class` (or `interface` modifier, Dart 3) for contracts; `class`
for DTOs:

```dart
abstract class TaskApi {
  Future<Task> createTask(CreateTaskInput input);
  Future<Paged<Task>> listTasks(ListTasksParams params);
...
Use typed exception classes; map to the universal REST envelope
(`{ code, message, details? }`) in the HTTP layer:
...
class ValidationException extends ApiException {
  ...
  ValidationException(this.details) : super('VALIDATION_ERROR', 'Validation failed');
}

class NotFoundException extends ApiException {
  NotFoundException(String id) : super('NOT_FOUND', 'Task $id not found');
}
```

## Boundary Validation

Validate at the boundary (`json_serializable` decode + manual /
`form_builder_validator`); internal code trusts the parsed value:

```dart
CreateTaskInput parseCreate(Map<String, dynamic> body) {
  final title = body['title'] as String?;
  if (title == null || title.isEmpty || title.length > 200) {
    throw ValidationException([FieldError('title', '1..200 chars')]);
  }
  return CreateTaskInput(title: title, description: body['description'] as String?);
}
```

## Additive Evolution

- New fields must be nullable or have a default so existing callers compile and
  old JSON decodes.
- Adding a required named param (`required this.x`) is breaking.
- For JSON DTOs, `json_serializable` ignores unknown keys by default — keep it.

## Variant Types

Use sealed classes (Dart 3) with exhaustive `switch`:
...
class InProgress extends TaskStatus {
  ...
  InProgress(this.assignee, this.startedAt);
}
class Completed extends TaskStatus {
  final DateTime completedAt;
  final String completedBy;
  Completed(this.completedAt, this.completedBy);
}
class Cancelled extends TaskStatus {
  final String reason;
  final DateTime cancelledAt;
  Cancelled(this.reason, this.cancelledAt);
}

String label(TaskStatus s) => switch (s) {
  Pending()                    => 'Pending',
  InProgress(:final assignee)  => 'In progress ($assignee)',
  Completed(:final completedAt) => 'Done on $completedAt',
...
Use extension types (Dart 3.3+) or small classes so distinct IDs are not
interchangeable:
