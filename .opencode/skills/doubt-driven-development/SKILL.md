...
- **Do NOT add this skill to a persona's `skills:` frontmatter.** A persona that follows Step 3 would spawn another persona — the orchestration anti-pattern explicitly forbidden by `.opencode/harness/rules/orchestration-patterns.md` ("personas do not invoke other personas").
...
In OpenCode, the role-based reviewers in `.opencode/agents/` start with isolated context by design — invoke via the `task` tool with the adversarial prompt above. See `.opencode/agents/` for the roster and per-domain match.
...
- **Repo orchestration rules** (`.opencode/harness/rules/orchestration-patterns.md`): this skill orchestrates from the main session. A persona calling another persona is anti-pattern B — see Loading Constraints above.
